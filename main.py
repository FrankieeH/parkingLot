...
def time_to_park():
	...
	for i in range(0,len(carArray)):
		...
		logger.info(f"Attempting to park Car in spot #: {randnum}")

		if total < len(lot1.parking_lot_array):
			carArray[i].park(lot1.parking_lot_array)
		else:
			logger.info(f"Parking Lot has reached full capacity. Unable to park Car with License Plate Number: {carArray[i].license_plate_num}, Sorry, come again another time!\n")

		total +=1


		#logger.info("Parking spots attempted to be  filled: ", total)
		#logger.info("Total parking spots available: ", lot1.total_parking_spots)
...
if __name__ == "__main__":
	...
	logger.info("Welcome to Frankie's Parking Lot Services\n")


	logger.info("First let's create the Parking Lot\n")
	...
	#logger.info out the parking lot to make sure it exists
	logger.info(f'\n{lot1},\n')
	...
	logger.info([str(i) for i in carArray])
